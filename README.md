...
Weight memory follows directly from the parameter count, so it is filled in for every row —
marked *est.* where derived rather than reported by the paper.

**Activation memory is not in this table**, because it needs the layer shapes and most papers do
not publish them. It could be computed for only two of these works, and both are given below rather
than left as a column that is empty eleven times.

| Work | Ch | Acc | Sens | Params | Precision | Weights | MACs |
|---|--:|--:|--:|--:|---|--:|--:|
| Chung 2024, *Front. Neurol.* | **1** | 98.18 % | **99.62 %** (event) / 96.76 % (seg) | 116 700 | FP32 | **454 KB** | — |
